...
function SettingRow({ label, description, children }) {
  return (
    <div className="flex flex-col gap-3 border-b border-slate-100 py-4 last:border-0 last:pb-0 first:pt-0 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
...
export default function Settings() {
  ...
  async function handleRefreshLeads() {
    ...
    try {
      ...
      setRefreshError(err.message || 'Failed to refresh leads');
    } finally {
      setRefreshing(false);
    }
  }
    ...
    <div className="w-full space-y-5 sm:space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-3 lg:gap-6">
        ...
        <Card title="Data & sync" subtitle="Lead records and refresh" icon={icons.database} className="lg:col-span-1">
          ...
          {refreshError && (
            <p className="mb-3 text-xs text-red-500">{refreshError}</p>
          )}
          <Button
            variant="secondary"
            size="sm"
